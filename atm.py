...
class Database():
    ...
    # 1.2 加入資料
    ## 1.2.1 User_Data
    def Insert_User_db(self,user_ID, user_Name, user_idCard, user_Password, user_Phone, user_Money=0):
      conn = sqlite3.connect('ATM_data.db')
      lock = 0
      cmd = "INSERT INTO USERS (ID,Name,idCard,Password,Phone,Money,Lock) VALUES (\'{}\',\'{}\',\'{}\',{},{},{},{})".format(user_ID, user_Name, user_idCard, user_Password, user_Phone, user_Money,lock)
      conn.execute(cmd)
      conn.commit()
      conn.close()
    ...
    def select_db(self,user_ID):
        conn = sqlite3.connect('ATM_data.db')
        money = conn.execute("SELECT Money FROM USERS WHERE ID = \'{}\'".format(user_ID))
        for i in money:	
          print('餘額:{}元'.format(i))	
          conn.commit()	
          conn.close()	
    ...
    def select_other_db(self,other_ID):
        conn = sqlite3.connect('ATM_data.db')
        money = conn.execute("SELECT Money FROM USERS WHERE ID = \'{}\'".format(other_ID))
        for i in money:	
    ...
    def withdraw(self,user_ID, with_money): 
        ...
        if user_money[0] - with_money > 0:
          conn.execute("UPDATE USERS SET Money = \'{}\' WHERE ID = \'{}\'".format(user_money[0] - with_money, user_ID))
    ...
    def wallet(self,user_ID, wallet_money): 
        ...
        conn = sqlite3.connect('ATM_data.db')
        user_money = db.select_db(user_ID)
        conn.execute("UPDATE USERS SET Money = \'{}\' WHERE ID = \'{}\'".format(user_money[0] + wallet_money, user_ID))
    ...
    def transfer_accounts(self,user_ID, other_ID, turn_money):
      ...
      if user_money[0] - turn_money > 0:
        conn.execute("UPDATE USERS SET Money = \'{}\' WHERE ID = \'{}\'".format(user_money[0] - turn_money, user_ID))
        conn.execute("UPDATE USERS SET Money = \'{}\' WHERE ID = \'{}\'".format(other_money[0] + turn_money, other_ID))
